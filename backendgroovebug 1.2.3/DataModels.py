...
class SimilarArtists(db.Model):
    ...
    similar_artists = db.StringListProperty(default = None)
    date_added = db.DateTimeProperty(auto_now=True)
""" --- """
...
## retrieve facebook music likes
    if fbID != None and fbAuth != None:
        ...
        if not fbuser:
            ...
            if fbArtistList != []:
                fbuser.fbArtistList = fbArtistList
            fbuser.put()
        if fbuser:
            ...
            facebookFavoritesJsonData = []
            for artist in facebookFavorites:
                artistDictEntry = {}
                artistDictEntry['name'] = artist
                artistDictEntry['magazine'] = 'http://' + str(hostUrl) + "/v1/magazine?artist=" + urllib.quote_plus(artistDictEntry['name'].encode('utf-8'))
                facebookFavoritesJsonData.append(artistDictEntry)
            facebookFavoritesJson = json.dumps(facebookFavoritesJsonData, separators=(',',':'))
            facebookFavoritesJson = facebookFavoritesJson.encode('utf-8')
            if currentUser is not None:
                currentUser.facebookFavorites = facebookFavorites
                currentUser.facebookFavoritesJson = facebookFavoritesJson
...
## Create favorites list from user library, sort them alphabetically, and create a new list with corresponding magazine links    
    ...
    groovebugFavoritesJsonData = []
    for artist in groovebugFavorites:
        artistDictEntry = {}
        artistDictEntry['name'] = artist
        artistDictEntry['magazine'] = 'http://' + str(hostUrl) + "/v1/magazine?artist=" + urllib.quote_plus(artistDictEntry['name'].encode('utf-8'))
        groovebugFavoritesJsonData.append(artistDictEntry)
    groovebugFavoritesJson = json.dumps(groovebugFavoritesJsonData, separators=(',',':'))
    groovebugFavoritesJson = groovebugFavoritesJson.encode('utf-8')

    if currentUser is None:
        currentUser = GroovebugUser(key_name = user, userId = user)
        currentUser.put()
##        currentUser.groovebugFavoritesJson = groovebugFavoritesJson
##        currentUser.put()
##        currentUser.artistListJson = artistListJson
##        currentUser.put()
...
## Initialize dictionary that will hold bio data
    ...
    ## Obtain echonest artist id in order to perform faster searches   
        searchBase = 'http://developer.echonest.com/api/v4/artist/biographies?api_key=269A92REW5YH3KNZW&'
        searchArgs = {'format' : 'json',}
        searchHelper = UrlFetchHelper(searchBase, searchArgs)
        bioJson = searchHelper.MakeSyncJsonFetch('name', artist)


    # Try loading Json doc from URL, if loading times out, try again up to
    # 'maxLookupRetry' times
        if bioJson == []:
            artistBio['status']['error'] = 'The echonest API request timed out'
            return artistBio

    ## Return dictionary with error message in status and blank artists array if artist
    ## not found
        elif bioJson['response']['status']['code'] == 5:
            artistBio['status']['error'] = 'artist not found'
            return artistBio

    # Otherwise, add artists corrected name to news dictionary and use echonest
    # artist Id to obtain news items from echonest
        else:
            if bioJson['response'].has_key('total'):
                if not bioJson['response']['total'] == 0:
                    for entry in bioJson['response']['biographies']:
                        if entry['site'] == 'wikipedia' or entry['site'] == 'Wikipedia':
                            artistBio['bios']['wikipedia'] = entry
                            artistBio['bios']['wikipedia']['generalBio'] = re.sub(r'(Early life)(.*)$|(Life and career)(.*)$|(History)(.*)$|(Band history)(.*)$', '', artistBio['bios']['wikipedia']['text'])
                            if len(artistBio['bios']['wikipedia']['generalBio']) < 100:
                                artistBio['bios']['wikipedia']['generalBio'] = artistBio['bios']['wikipedia']['text'][:600] + '...'
                            elif len(artistBio['bios']['wikipedia']['generalBio']) > 600:
                                artistBio['bios']['wikipedia']['generalBio'] = artistBio['bios']['wikipedia']['generalBio'][:600] + '...'
                            else:
                                artistBio['bios']['wikipedia']['generalBio'] += '...'
                        if entry['site'] == 'last.fm' or entry['site'] == 'Last.fm':
                            artistBio['bios']['last.fm'] = entry
                        if artistBio['bios'].has_key('last.fm') and artistBio['bios'].has_key('wikipedia'):
                            break
    return artistBio
""" --- """
...
def GetFriendsTileScore(friendArtistList, artistList):
    ...
    if len(gArtArr) != 0:
        ...
        simLists = echonest.GetSimilarArtists(friendArtistList)
...
# Encode results into unicode which can be parsed by API
    ...
    artist = urllib.quote_plus(uArtist)
...
# about the search
    ...
    data = memcache.get('artistNews' + artist)
    if data is not None:
...
# Obtain blinkfire news items
    ...
    if blink and  itemCnt < maxResults:
        ...
        searchBase = 'http://music-news.blinkfirelabs.com/news/v1?key=foo&lr=lang_en&max_results=' + str(maxResults) + '&q=' + artist
...
# If there are not enough entries, obtain more entries from echonest
    ...
    if itemCnt < maxResults:
        searchBase = 'http://developer.echonest.com/api/v4/artist/news?api_key=269A92REW5YH3KNZW&'
        searchArgs = {'format' : 'json',
                      'results' : str(maxResults),
                      'high_relevance' : 'true',}
        searchHelper = UrlFetchHelper(searchBase, searchArgs)
        newsJson = searchHelper.MakeSyncJsonFetch('name', uArtist)
        if newsJson == []:
            if not len(artistNews['stories']) > 0:
                artistNews['status']['error'] = 'The echonest API call timed out'
                #return artistNews

    # Return dictionary with error message in status and blank artists array if artist
    # not found
        elif newsJson['response']['status']['code'] == 5:
            if not len(artistNews['stories']) > 0:
    ...
    # artist Id to obtain news items from echonest
            ...
            if newsJson['response'].has_key('news'):
                if not newsJson['response']['news'] == []:
                    artistFound = True
                    echonestNews = newsJson['response']['news']
                    for entry in echonestNews:
                        newsItem = {}
                        newsItem['title'] = re.sub(r'<.*?>', '', unescape(entry['name']))
                        sameTitle = False
                        for titleChk in artistNews['stories']:
                        ...
                        if not sameTitle:
                            try:
                                newsItem['summary'] = re.sub(r'<.*?>', '', unescape(entry['summary']))
                                if len(newsItem['summary']) > 350:
                                    newsItem['summary'] = newsItem['summary'][:350] + '...'
...
# news stories wasnt obtained from blinkfire and echonest
    itemCnt = len(promoNews['stories']) + len(artistNews['stories'])
    if itemCnt < maxResults:
        googleSearchUrl = 'https://www.googleapis.com/customsearch/v1?key=AIzaSyD18jSXZ1WtHZZrn9dGaP9ltLfXerY4Ons&cx=015060547345235644646:o3edx1lq9um&q=' + artist
        try:
    ...
    if len(artistNews['stories']) > 0:    
        ...
        data = memcache.get('artistNews' + artist)
        if data is not None:
            memcache.set('artistNews' + artist, artistNews, 10800)
        else:
            memcache.add('artistNews' + artist, artistNews, 10800)

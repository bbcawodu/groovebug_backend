...
class CorrectionHandler(webapp.RequestHandler):
    def get(self):
        ...
        artistEntries = models.ArtistCorrection.all()
        for artistEntry in artistEntries:
            ...
            if timeDifference.days >= 7:
                self.response.out.write(artistEntry.uncorrected_name + ' deleted <br/>')
                artistEntry.delete()
            else:
                self.response.out.write(artistEntry.uncorrected_name + ' NOT deleted <br/>')

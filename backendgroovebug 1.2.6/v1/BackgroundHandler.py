...
# Initiallize the dictionary that will eventually be converted to json
    ...
    outStr = str(loopEnd) + ' Echonest Image URLs return'
    logging.debug(outStr)
    if artistImgCnt < minPics:
        outStr = 'Artist image count was ' + str(artistImgCnt) + ' which is less that min pics'
        logging.debug(outStr)
        rpcs = []
        currCallList = []
        ...
        for i, entry in enumerate(finalPreJson['images']):
            ...
            if imgUrl not in currCallList:
                ...
                outStr = str(imgUrl) + ' was added to current url call list, this time it is working'
                logging.debug(outStr)
                logging.debug(str(currCallList))
                ...
                if imgChkExists['imgUrl'] == '':
                    ...
                    #print 'array rpc call val: ', rpcUrlArr[rpc]['url'], rpcUrlArr[rpc]['license']
                    rpcs.append(rpc)
                else:
                ...
                outStr = str(imgUrl) + ' was already in the current url call list'
                logging.debug(outStr)
        ...
        outStr = 'Artist image count was ' + str(artistImgCnt) + ' which is more that min pics'
        logging.debug(outStr)

...
class CorrectionHandler(webapp.RequestHandler):
    def get(self):
        ...
        logging.info('Getting keys for outdated similar artist entries')
        outdatedEntryKeysQuery = db.GqlQuery("SELECT __key__ FROM SimilarArtists " +
        ...
        if outdatedEntryKeys != []:
            logging.info('Getting entries for ' + str(len(outdatedEntryKeys)) + ' outdated similar artists')
            artistEntries = models.SimilarArtists.get(outdatedEntryKeys)
            
            for index, artistEntry in enumerate(artistEntries):
                artistsToBeUpdated.append(artistEntry.artist_name)
                memcache.delete('similarArtistCache' + artistEntry.artist_name)
                artistEntry.delete()
                if len(artistsToBeUpdated) >= 200 or index+1 == len(artistEntries):
                    ...
                    updatedArtists = echonest.GetSimilarArtists(artistsToBeUpdated, deadline=20, maxFails = 3*len(artistsToBeUpdated))
                    artistsToBeUpdated[:] = []
        else:
            logging.info('No outdated similar artists found')
        logging.info('FINISH') 
""" --- """

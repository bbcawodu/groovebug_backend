...
from google.appengine.api import urlfetch, memcache, datastore_types
from google.appengine.runtime import DeadlineExceededError
from django.utils import simplejson as json
import echonestlib as echonest
from operator import itemgetter
import urllib2, urllib, re, htmlentitydefs, logging, unicodedata, math, facebook
...
class FacebookUser(db.Model):
    ...
    fbArtistListVerified = db.BooleanProperty(default = False)
    ...
    friendsIDList = db.StringListProperty(default = None)
    friendsToChkList = db.StringListProperty(default = None)
""" --- """
...
class GroovebugUser(db.Model):
    ...
    facebookFavoritesAlreadyAdded = db.StringListProperty(default = None)
    facebookFavoritesJson = db.TextProperty(default = None)
    groovebugFavoritesJson = db.TextProperty(default = None)
    facebookProfile = db.ReferenceProperty(FacebookUser, collection_name = 'groovebug_user_set')
...
class ArtistSocial(db.Model):
    ...
    tumblr = db.StringProperty()
    myspace = db.StringProperty()
    soundcloud = db.StringProperty()
    wikipedia = db.StringProperty()
    youtube = db.StringProperty()
    lastfm = db.StringProperty()
    purevolume = db.StringProperty()
...
class ArtistWork(db.Model):
    artistName = db.StringProperty(required=True)
    imagesToExclude = db.StringListProperty()
    newsToExclude = db.StringListProperty()
...
""" This function recieves user library data from an HTML POST and a list of
    corrected artists names that correspond to the data and maps corrected artist names
    to entries in the user library to their appropriate artist list."""
def MapArtistsV2(libraryList, correctedLibraryList, favoritesList, correctedFavsList):
    libraryMappings = {}
    favoritesMappings = {}

    if libraryList != []:
        for index, artist in enumerate(libraryList):
            correctedName = correctedLibraryList[index]
            if correctedName != None:
                if not libraryMappings.has_key(correctedName):
                    libraryMappings[correctedName] = [artist]
                else:
                    if not artist in libraryMappings[correctedName]:
                        libraryMappings[correctedName].append(artist)

    if favoritesList != []:
        for index, artist in enumerate(favoritesList):
            correctedName = correctedFavsList[index]
            if correctedName != None:
                if not favoritesMappings.has_key(correctedName):
                    favoritesMappings[correctedName] = [artist]
                else:
                    if not artist in favoritesMappings[correctedName]:
                        favoritesMappings[correctedName].append(artist)
...
## Log the total number of artists in the users library to the google app engine dashboard
    logging.info('number of artists: ' + str(len(userData['library'])))

## Parse user artist list, use echonest library to correct names, and map the artist names to corrected names in appropriate list.
    recievedLibrarylist = []
    correctedLibraryList = []
    recievedFavslist = []
    correctedFavsList = []
    if userData.has_key('library'):
        for artist in userData['library']:
            recievedLibrarylist.append(artist['name'])
        correctedLibraryList = echonest.CorrectArtistNames(recievedLibrarylist)

    if userData.has_key('favorites'):
        for artist in userData['favorites']:
            recievedFavslist.append(artist)
        correctedFavsList = echonest.CorrectArtistNames(recievedFavslist)
                  
    libraryMappings, favoritesMappings = MapArtistsV2(recievedLibrarylist, correctedLibraryList, recievedFavslist, correctedFavsList)
...
## creat a link to the facebook friends handler
    if fbAuth != None:
        finalPreJson['friends'] = 'http://' + str(hostUrl) + "/v2/friends?user=" + str(user) + '&fbauth=' + fbAuth
        graph = facebook.GraphAPI(fbAuth)
        profile = graph.get_object("me")
        finalPreJson['facebook user name'] = profile['name']
        
        facebookFavorites = []
        facebookUsers = FacebookUser.gql("WHERE access_token  = :accessToken", accessToken = fbAuth)
        if facebookUsers is not None:
            for facebookUser in facebookUsers:
                if facebookUser.fbArtistList != None:
                    facebookFavorites = facebookUser.fbArtistList
                    if not facebookUser.fbArtistListVerified:
                        correctedFacebookFavorites = echonest.CorrectArtistNames(facebookFavorites)
                        fbArtistListNew = []
                        for artist in correctedFacebookFavorites:
                            if artist != None:
                                fbArtistListNew.append(artist)
                        fbArtistListNew = sorted(fbArtistListNew, key = lambda artist: natural(artist))
                        facebookFavorites = fbArtistListNew
                    facebookUser.fbArtistList = facebookFavorites
                    facebookUser.fbArtistListVerified = True
                    facebookUser.put()
        else:
            musicLikes = graph.get_connections(profile["id"], "music")
            fbArtistList = []
            ...
            facebookUser = FacebookUser(key_name = profile["id"],
                                        name = profile["name"],
                                        profile_url = profile["link"],
                                        fbArtistList = facebookFavorites,
                                        fbArtistListVerified = True,
                                        access_token = fbAuth)
            facebookUser.put()
            
        if facebookFavorites != []:
            facebookFavoritesAlreadyAdded = []
            if currentUser:
                if currentUser.facebookFavoritesAlreadyAdded != None:
                    facebookFavoritesAlreadyAdded = currentUser.facebookFavoritesAlreadyAdded
            facebookFavoritesAlreadyAddedSet = set(facebookFavoritesAlreadyAdded)
            facebookFavoritesSet = set(facebookFavorites) - facebookFavoritesAlreadyAddedSet
            groovebugFavoritesSet = set(groovebugFavorites)
            numberOfLikesAdded = len(facebookFavoritesSet - groovebugFavoritesSet)
            if numberOfLikesAdded != 0:
                groovebugFavorites = facebookFavoritesSet | groovebugFavoritesSet
                groovebugFavorites = sorted(groovebugFavorites, key = lambda artist: natural(artist))
                finalPreJson['import message'] = 'Imported ' + str(numberOfLikesAdded) + ' favorites from your Facebook likes'
            facebookFavoritesAlreadyAddedSet = facebookFavoritesAlreadyAddedSet | facebookFavoritesSet
    else:
        finalPreJson['facebook user name'] = ''
        finalPreJson['friends'] = None
    
    favoritesChanged = False
    finalGroovebugFavoritesSet = set(groovebugFavorites)
    postedGroovebugFavoritesSet = set(correctedFavsList)
    numOfFavsChanged = len(finalGroovebugFavoritesSet - postedGroovebugFavoritesSet)
    numOfFavsChanges2 = len(postedGroovebugFavoritesSet - finalGroovebugFavoritesSet)
    if numOfFavsChanged != 0 or numOfFavsChanges2 != 0:
        favoritesChanged = True
    if finalPreJson['facebook user name'] != '':
        if favoritesChanged == True:
            finalPreJson['facebook user name'] = finalPreJson['facebook user name'] + 'update=true'
        else:
            finalPreJson['facebook user name'] = finalPreJson['facebook user name'] + 'update=false'
    ...
    if finalPreJson.has_key('import message'):
        currentUser.facebookFavoritesAlreadyAdded = list(facebookFavoritesAlreadyAddedSet)
    currentUser.put()
...
##.    finalPreJson['splash html'] = 'http://' + str(hostUrl) + '/static/splashes/Update1.2.7.html'
    ...
    logging.info('In The middle of GetSavedUserData')
    if currentUser is not None:
...
## If there is an authentication token in the call to the home handler,
## create a link to the facebook friends handler, and add facebook music likes to favorites
        if fbAuth != None:
            finalPreJson['friends'] = 'http://' + str(hostUrl) + "/v2/friends?user=" + str(user) + '&fbauth=' + fbAuth + 'update=false'
            graph = facebook.GraphAPI(fbAuth)
            profile = graph.get_object("me")
            finalPreJson['facebook user name'] = profile['name']
...
# about the search
    ...
    exclList = []
    exclFile = ArtistWork.get_by_key_name(artistName)
    if exclFile:
        if exclFile.newsToExclude:
            exclList = exclFile.newsToExclude
            ...
            if data.has_key('stories'):
                if data['stories'] is not None:
                    if len(data['stories']) > 0:
                        if not(len(promoNews['stories']) > 0):
                            return data
                        else:
...
# Obtain blinkfire news items
    ...
    if blink and  itemCnt < maxResults:
        ...
        try:
            ...
            #logging.info(str(len(newsJson)) + ' news articles found on blinkfire')
        except Exception, error:
            newsJson = []
            #logging.info('There was a problem with getting news articles from blinkfire')
            ...
            for entry in newsJson:
                ...
                if not sameTitle:
                    try:
                        ...
                        logging.debug('Blinkfire caused error - ' + str(error))
                        pass

            #logging.info(str(len(artistNews['stories'])) + ' news articles used from blinkfire')
# If there are not enough entries, obtain more entries from echonest
    ...
    if itemCnt < maxResults:
        ...
        if echonestNewsItems != []:
            artistFound = True
            #logging.info(str(len(echonestNewsItems)) + ' news articles found on echonest')
...
# news stories wasnt obtained from blinkfire and echonest
    ...
    if itemCnt < maxResults:
        ...
        try:
            ...
            if googleJson == []:
                logging.info(str(len(artistNews['stories'])) + ' news articles found on google')
                if not len(artistNews['stories']) > 0:
                    artistNews['status']['error'] = 'The google search API call timed out'
                    #return artistNews
            if googleJson.has_key('items'):
                counter = 0
                while itemCnt < maxResults and counter <= len(googleJson['items'])-1:
                    newsItem = {}
                    ...
                    if not sameTitle:
                        try:
                            exclUrl = googleJson['items'][counter]['link']
                            exclUrl = exclUrl.strip().lower()
                            if exclUrl not in exclList:
...
def getArtistSocial(artist):
    artistEnc = urllib.quote_plus(artist)
    socialFile = ArtistSocial.gql("WHERE artistName = :1 LIMIT 1", artistEnc).fetch(1)
    if len(socialFile) > 0:
        if socialFile[0].twitter:
            if socialFile[0].twitter != '':
                return socialFile[0].twitter
    else:
        socialFile = ArtistSocial.get_or_insert(key_name = str(artistEnc), artistName = artistEnc)

...
class RedirectCompositeHandler(webapp.RequestHandler):
    def post(self):
        ...
        if compositeType:
            compositeTypeOptionString =  compositeTypeOptionString + ' value="' + compositeType + '"'
        ...
        if self.request.get('verify') == 'yes':
            echonestCorrectedArtistList = echonest.CorrectArtistNames(artistList)
            correctedArtists = []
            errorArtists = []
            for index, artist in enumerate(echonestCorrectedArtistList):
                if artist is None:
                    errorArtists.append(artistList[index])
                else:
                    correctedArtists.append(artist)

    ...
    try:
        ...
        if profileResults.status_code == 200:
            ...
            for profile in batchOfProfiles:
                ...
                if batchOfProfiles[profile].has_key("link"):
                    profileEntry['url'] = batchOfProfiles[profile]["link"]
                else:
                    logging.info('Friend ' + str(batchOfProfiles[profile]["name"]) + ' has a link issue')
                    logging.info('The Profile - ' + str(batchOfProfiles[profile]))
                    profileEntry['url'] = "http://www.facebook.com/" + batchOfProfiles[profile]["id"]
                ...
                if friendFacebookUser is None:
                    profileEntry['likes verified'] = False
                    friendFacebookUser = models.FacebookUser(key_name = batchOfProfiles[profile]["id"],
                                                             name = batchOfProfiles[profile]["name"],
                                                             profile_url = profileEntry['url'],
                                                             fbArtistListVerified = False,
                                                             fbArtistList = [])
                else:
                    friendFacebookUser.name = batchOfProfiles[profile]["name"]
                    friendFacebookUser.profile_url = batchOfProfiles[profile]["link"]
                    if not friendFacebookUser.fbArtistList:
                        profileEntry['likes verified'] = False
                        friendFacebookUser.fbArtistList = []
                    else:
                        if friendFacebookUser.fbArtistListVerified:
                            profileEntry['likes verified'] = True
                            profileEntry['music likes'] = friendFacebookUser.fbArtistList

...
class ImageFindArtist(ImageWorkBaseHandler):
    ...
    def get(self):
        ...
        artists = models.ArtistImages.all().order('+artistName')
...
class ImageRedirect(ImageWorkBaseHandler):
    ...
    def post(self):
        ...
        elif self.request.get('excludeImage'):
            imgKey = self.request.get('excludeImage')
            imgFile = db.get(imgKey)
            if imgFile:
                artistName = imgFile.artistName
                queryName = artistName.encode('utf-8')
                queryName = urllib.quote_plus(queryName)
                origUrl = imgFile.artistOrigImgUrl
                actionMsg = DeleteImages(imgFile)
                if actionMsg == 'Image Deleted!!!':
                    wrkFile = models.ArtistWork.get_or_insert(key_name = str(artistName), artistName = artistName)
                    exclList = []
                    origUrl = origUrl.strip().lower()
                    if wrkFile.imagesToExclude:
                        if not wrkFile.imagesToExclude is None:
                            exclList = wrkFile.imagesToExclude
                    
                    if origUrl not in exclList:
                        exclList.append(origUrl)
                    wrkFile.imagesToExclude = exclList
                    wrkFile.put()
                    actionMsg = 'Image Excluded!!!'

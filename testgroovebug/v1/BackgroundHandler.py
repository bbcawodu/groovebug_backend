            ...
            if result.status_code == 200:
                if self.artistImgCnt < MAX_PICS:
                    ...
                    if imgUrlRslt['imgUrl'] != '':
                        self.artistImgCnt += 1
                        outStr = 'Successful image #' + str(self.artistImgCnt) + ' - ' + str(imgUrl)
                        logging.debug(outStr)
                    else:
                        outStr = 'Unsuccessful image #' + str(self.artistImgCnt) + ' - ' + str(imgUrl)
                        logging.debug(outStr)
                else:
                    outStr = 'Limit ' + str(MAX_PICS) + 'reached but still checking - ' + str(imgUrl)
                    logging.debug(outStr)
            else:
                outStr = 'Failed url - ' + str(rpcUrlArr[rpc]['url'])
                logging.debug(outStr)
        except Exception, error:
            #print error
            imgUrlRslt = {'imgUrl': '', 'reducedUrl' : '', 'license' : {}, 'thumbUrl' : ''}
            outStr = 'Failed url - ' + str(rpcUrlArr[rpc]['url']) + ' with exception - ' + str(error)
            logging.debug(outStr)
...
# Initiallize the dictionary that will eventually be converted to json
    ...
    cachedUrlList = []
    #echoError = False
    msngImgAtt = 'http://' + str(hostUrl) + '/v1/imgatt?imgid=noimage'
    if artistName:
        cachedImgs = getManuallyStored(artistName)
        outStr = str(len(cachedImgs)) + ' manually stored images'
        logging.info(outStr)
        for cEntry in cachedImgs:
            if cEntry['url']:
                cachedUrlList.append(cEntry['url'])
        cachedUrlSet = set(cachedUrlList)
        #logging.info(cachedUrlList)
        ...
        # Perform lookup of background images for given artist
            ...
            backgroundArgs = { 'format' : 'json',
                           'results' : '100',}
            backgroundUrl = str(backgroundBase) + apiKey + str(urllib.urlencode(backgroundArgs)) + '&name=' + artistName
            backgroundJson = RetryLoadJson(backgroundUrl)
            #return json.dumps(backgroundJson, separators=(',',':'))
            if backgroundJson == []:
        ...
        # Return blank arrays for everything if artist isnt found
            ...
            if backgroundJson['response']['status']['code'] > 0:
                ...
                backUrlAdd = {'imgatt' : msngImgAtt, 'url' : 'http://' + str(hostUrl) + '/htmlimages/backgrounds/' + str(defBack),
    ...
    dsImgList = chkImgCnt(artistName)
    dsImgSet = set(dsImgList)

    imgEchoList = []
    for i, entry in enumerate(finalPreJson['images']):
        imgUrl = finalPreJson['images'][i]['url']
        imgUrl = imgUrl.strip().lower()
        imgEchoList.append(imgUrl)

    imgEchoSet = set(imgEchoList)    
    outStr = str(len(imgEchoSet)) + ' Echonest Image URLs return'
    logging.debug(outStr)
    

    echoImgsInDS = imgEchoSet & dsImgSet
    outStr = str(len(echoImgsInDS)) + ' Echonest Image URLs are already in the datastore'
    logging.debug(outStr)
    artistEchoImgMatchCnt = len(echoImgsInDS)

    echoImgsNotInDS = imgEchoSet - dsImgSet
    outStr = str(len(echoImgsNotInDS)) + ' Echonest Image URLs not in the datastore'
    logging.debug(outStr)

    delFromDS = dsImgSet - imgEchoSet
    outStr = str(len(delFromDS)) + ' Images in datastore that can be deleted'
    logging.debug(outStr)

    artistImgCnt = artistEchoImgMatchCnt + len(cachedUrlSet)
    
    if artistImgCnt < minPics:
        outStr = 'Artist (up to date with EchoNest) image count was ' + str(artistImgCnt) + ' which is less that min pics'
        logging.debug(outStr)
        rpcs = []
        currCallList = []
        exclList = []
        exclFile = models.ArtistWork.get_by_key_name(artistName)
        if exclFile:
            if exclFile.imagesToExclude:
                exclList = exclFile.imagesToExclude
        ...
        for i, entry in enumerate(finalPreJson['images']):
            imgUrl = finalPreJson['images'][i]['url']
            imgUrl = imgUrl.strip().lower()
            if imgUrl not in currCallList and imgUrl not in exclList:
                currCallList.append(imgUrl)
                imgChkExists = chkExistsLic(finalPreJson['images'][i], artistName)
                if imgChkExists['imgUrl'] == '':
                    rpc = uCalls.fetch_asynchronously(finalPreJson['images'][i]['url'])
                    rpcUrlArr[rpc] = {'imgid' : '', 'url' : finalPreJson['images'][i]['url'],
                                      'license' : finalPreJson['images'][i]['license'],
                                      'artist' : artistName, 'imgUrl' : '', 'reducedUrl' : '', 'thumbUrl' : ''}
                    outStr = 'rpc call license data was - ' + str(finalPreJson['images'][i]['license'])
                    logging.debug(outStr)
                    outStr = 'for url - ' + str(finalPreJson['images'][i]['url'])
                    logging.debug(outStr)
                    rpcs.append(rpc)
                else:
                    #outStr = ' - but it already existed in the datastore'
                    #logging.debug(outStr)
                    finalPreJson['images'][i]['imgatt'] = 'http://' + str(hostUrl) + '/v1/imgatt?imgid=' + str(imgChkExists['imgid'])
                ...
                #outStr = str(imgUrl) + ' was already in the current url call or exclude list'
                #logging.debug(outStr)
            ...
            for k, entry in enumerate(finalPreJson['images']):
                if finalPreJson['images'][k]['url'] == rpcUrlArr[rpc]['url']:
                    if rpcUrlArr[rpc]['imgUrl'] == '':
                        ...
                        finalPreJson['images'][k]['imgatt'] = 'http://' + str(hostUrl) + '/v1/imgatt?imgid=' + str(rpcUrlArr[rpc]['imgid'])
            ...
            if entry['url'] != {}:
                ...
                if chkRslt != {}:
                    if chkRslt['imgUrl'] not in currUrlList:
                        ...
                        outStr = 'Url ' + str(entry['url']) + ' was found in the datastore as ' + str(chkRslt['imgUrl'])
                        logging.debug(outStr)
                        finalPreJson['images'][l]['imgatt'] = 'http://' + str(hostUrl) + '/v1/imgatt?imgid=' + str(chkRslt['imgid'])
                        ...
                        outStr = 'Url ' + str(entry['url']) + ' was a duplicate'
                        logging.debug(outStr)
                        finalPreJson['images'][l]['imgatt'] = ''
        ...
        outStr = 'Artist (up to date with EchoNest) image count was ' + str(artistImgCnt) + ' which is more that min pics'
        logging.debug(outStr)
        outStr = 'Current URL List - ' + str(currUrlList)
        logging.debug(outStr)
        for i, entry in enumerate(finalPreJson['images']):
            if entry['url'] != {}:
                chkRslt = chkExistsLic(entry, artistName)
                if chkRslt['imgUrl'] != '':
                    if chkRslt['imgUrl'] not in currUrlList:
                        currUrlList.append(chkRslt['imgUrl'])
                        outStr = 'Url ' + str(entry['url']) + ' was found in the datastore as ' + str(chkRslt['imgUrl'])
                        logging.debug(outStr)
                        finalPreJson['images'][i]['imgatt'] = 'http://' + str(hostUrl) + '/v1/imgatt?imgid=' + str(chkRslt['imgid'])
                        ...
                        outStr = 'Url ' + str(entry['url']) + ' was a duplicate'
                        logging.debug(outStr)
                        finalPreJson['images'][i]['imgatt'] = ''
                        finalPreJson['images'][i]['url'] = ''
                        finalPreJson['images'][i]['license'] = {}
                        finalPreJson['images'][i]['license']['thumbnail'] = ''
                        finalPreJson['images'][i]['reduced'] = ''
                else:
                    outStr = 'Url check failed and got this - ' + str(chkRslt)
                    logging.debug(outStr)
                    finalPreJson['images'][i]['imgatt'] = ''
                    finalPreJson['images'][i]['url'] = ''
                    finalPreJson['images'][i]['license'] = {}
    ...
    #    pass
    if len(delFromDS) > 0:
        delUrlList = list(delFromDS)
        delUrls = json.dumps(delUrlList)
        taskqueue.add(url='/v1/backgroundsdelworker', params={'delUrlList': delUrls})
        logging.info('Added task queue for ' + str(len(delUrlList)) + ' images')
        logging.info('The list to check is ' + str(delUrlList))
...
# Perform check for optimal size image
    ...
    imgBlobRslt = {'imgid': '', 'imgUrl': '', 'reducedUrl' : '', 'license' : {}, 'thumbUrl' : ''}
...
#            Size the image to a thumbnail and write the thumbnail to the blobstore
            ...
            imgFile.put()
            imgBlobRslt['imgid'] = imgFile.key()
            rpcUrlArr[rpc]['imgid'] = imgFile.key()
            imgCnt = imgCnt + 1
    except Exception, error:

        imgBlobRslt = {'imgid': '', 'imgUrl': error, 'reducedUrl' : '', 'license' : {}, 'thumbUrl' : ''}
...
def chkExists(url):
    ...
    if len(url)> 500:
        return {'imgid': '', 'imgUrl': '', 'license': {}, 'thumbUrl': '', 'reducedUrl': ''}
    ...
    if (len(result) > 0):
        try:
            if result[0].artistImgLicense:
                ...
                imgLic = {}
            return {'imgid': result[0].key(), 'imgUrl': result[0].artistImgUrl, 'reducedUrl': result[0].artistReducedImgUrl,
                    'license': imgLic, 'thumbUrl': result[0].artistThmbUrl}
        except Exception, error:
            outStr = 'chkExists Error - ' + str(error)
            logging.debug(outStr)
...
""" Section ending with "---" defines checks to see if an entry already exists for the image requested"""
def chkExistsLic(entry, artistName):

    if len(entry['url'])> 500:
        return {'imgid': '', 'imgUrl': '', 'license': {}, 'thumbUrl': '', 'reducedUrl': ''}
        
    results = models.ArtistImages.gql("WHERE artistOrigImgUrl = :1 LIMIT 1", entry['url']).fetch(1)

    if (len(results) > 0):
        try:
            if results[0].artistImgLicense:
                imgLic = results[0].artistImgLicense
                if imgLic == {}:
                    imgLic['url'] = entry['license']['url']
                    imgLic['type'] = entry['license']['type']
                    imgLic['attribution'] = entry['license']['attribution']
                    results[0].artistImgLicense = imgLic
                    results[0].put()
                else:
                    if imgLic.has_key('url'):
                        ...
                        imgLic['url'] = entry['license']['url']
                    if imgLic.has_key('type'):
                        if imgLic['type'] == '':
                            imgLic['type'] = entry['license']['type']
                    else:
                        imgLic['type'] = entry['license']['type']
                    if imgLic.has_key('attribution'):
                        if imgLic['attribution'] == '':
                            imgLic['attribution'] = entry['license']['attribution']
                    else:
                        imgLic['attribution'] = entry['license']['attribution']
                    results[0].artistImgLicense = imgLic
            ...
            if not results[0].dateAdded:
                results[0].dateAdded = datetime.utcnow().date()
                results[0].put()
            #return results[0].dateAdded
        
            return {'imgid': results[0].key(), 'imgUrl': results[0].artistImgUrl, 'reducedUrl': results[0].artistReducedImgUrl,
                    'license': imgLic, 'thumbUrl': results[0].artistThmbUrl}
        except Exception, error:
            outStr = 'chkExistsLic Error - ' + str(error)
            logging.debug(outStr)
            return error
            if results[0].artistImgUrl:
                return {'imgid': results[0].key(), 'imgUrl': results[0].artistImgUrl, 'reducedUrl': results[0].artistReducedImgUrl,
                        'license': {}, 'thumbUrl': results[0].artistThmbUrl}
            else:
                return {'imgid': '', 'imgUrl': '', 'license': {}, 'thumbUrl': '', 'reducedUrl': ''}
    else:
        results = models.ArtistImages.gql("WHERE artistName = :1 LIMIT 50", artistName).fetch(50)
        if (len(results) > 0):
            for result in results:
                if result.artistOrigImgUrl.lower() == entry['url'].lower():
                    #result.artistOrigImgUrl = result.artistOrigImgUrl.lower()
                    if result.artistImgLicense:
                        imgLic = result.artistImgLicense
                        if imgLic == {}:
                            imgLic['url'] = entry['license']['url']
                            imgLic['type'] = entry['license']['type']
                            imgLic['attribution'] = entry['license']['attribution']
                            result.artistImgLicense = imgLic
                        else:
                            if imgLic.has_key('url'):
                                if imgLic['url'] == '':
                                    imgLic['url'] = entry['license']['url']
                    ...
                    if not results[0].dateAdded:
                        result.dateAdded = datetime.utcnow().date()

                    result.put()
                
                    return {'imgid': result.key(), 'imgUrl': result.artistImgUrl, 'reducedUrl': result.artistReducedImgUrl,
                            'license': imgLic, 'thumbUrl': result.artistThmbUrl}
...
def chkDBExists(url):
    ...
    if (len(result) > 0):
        try:
            ...
            outStr = 'chkDBExists Error - ' + str(error)
            logging.debug(outStr)
            if result[0].artistImgUrl:
                return {'imgid': result[0].key(), 'imgUrl': result[0].artistImgUrl, 'reducedUrl': result[0].artistReducedImgUrl,
                        'license': {}, 'thumbUrl': result[0].artistThmbUrl}
...
def chkDBExistsLic(entry):
    ...
    if (len(result) > 0):
        try:
            ...
            outStr = 'chkDBExistsLic Error - ' + str(error)
            logging.debug(outStr)
            if result[0].artistImgUrl:
                return {'imgid': result[0].key(), 'imgUrl': result[0].artistImgUrl, 'reducedUrl': result[0].artistReducedImgUrl,
                        'license': {}, 'thumbUrl': result[0].artistThmbUrl}
...
def chkImgCnt(artist):
    ...
    if (len(results) > 0):
        for entry in results:
            if entry.artistOrigImgUrl:
                if entry.artistOrigImgUrl != 'Manually Replaced' and entry.artistOrigImgUrl != 'Manually Added':
                    dsUrlList.append(entry.artistOrigImgUrl.lower())
    #return len(results)
    return dsUrlList
            ...
            if tmpLic.has_key('type'):
                if tmpLic['type']:
                    if tmpLic['type'] == 'Manually Replaced' or tmpLic['type'] == 'Manually Added':
                        ...
                        imgAtt = 'http://' + str(hostUrl) + '/v1/imgatt?imgid=' + str(entry.key())   
                        addEntry = {'imgatt': imgAtt, 'url': entry.artistImgUrl, 'reduced': tmpReduced, 'license': tmpLic}
...
class BackgroundsDelWorker(webapp.RequestHandler):        
    def post(self, delUrlList): # should run at most 1/s
        logging.info('It got to the POST') 
        imgDelUrlList = json.loads(self.request.get('delUrlList'))
        if len(imgDelUrlList) > 0:
            self.delImgs(imgDelUrlList)
        else:
            logging.info('There were no URLs to delete') 

    def delImgs(self, imgDelUrlList):
        logging.info('It got to the delImgs')

        for delUrl in imgDelUrlList:
            logging.info('Checking for image URL - ' + str(delUrl) + ' in the datastore') 
            imgFiles = models.ArtistImages.gql("WHERE artistOrigImgUrl = :1 LIMIT 10", delUrl).fetch(10)
            if (len(imgFiles) > 0):
                logging.info('It found ' + str(len(imgFiles)) + ' for that URL')
                        ...
                        if imgFile.artistReducedImgUrl:
                            if imgFile.artistReducedImgUrl != '':
                                if imgFile.artistReducedImgBlobKey:
                                    if imgFile.artistReducedImgBlobKey != '':
                                        logging.info('This would delete the reduced image blobstore entry - ' + str(imgFile.artistReducedImgBlobKey)) 
                        ...
                        if imgFile.artistImgUrl:
                            if imgFile.artistImgUrl != '':
                                if imgFile.artistImgBlobKey:
                                    if imgFile.artistImgBlobKey != '':
                                        logging.info('This would delete the main image blobstore entry - ' + str(imgFile.artistImgBlobKey)) 
                                        #blobstore.delete(imgFile.artistImgBlobKey)
                                        #imgFile.artistImgUrl = ''
                                        #imgFile.put()
                            else:
                                logging.info('Empty Image URL')

                        else:
                            logging.info('Image URL Is Missing')
                            
                        if imgFile.artistThmbUrl:
                            if imgFile.artistThmbUrl != '':
                                if imgFile.artistThmbBlobKey:
                                    if imgFile.artistThmbBlobKey != '':
                                        logging.info('This would delete the thumbnail blobstore entry - ' + str(imgFile.artistThmbBlobKey)) 
                                        #blobstore.delete(imgFile.artistThmbBlobKey)
                                        #imgFile.artistThmbUrl = ''
                                        #imgFile.put()
                            else:
                                logging.info('Empty Thumbnail URL')

                        else:
                            logging.info('Thumbnail URL Is Missing')

                        logging.info('This would delete datastore entry - ' + str(imgFile.key()) + ' for artist - ' + str(artistName))      
                        #imgFile.delete()
                        #actionMsg = 'Image Deleted!!!'
                    except Exception, error:
                        outStr = 'delImgs Error - ' + str(error)
                        logging.debug(outStr)

                    

# URL to source file mappings
application = webapp.WSGIApplication([('/v1/backgroundsdelworker(.*)', BackgroundsDelWorker),
                                      ('/v1/backgrounds(.*)', BackgroundHandler),],
                                       debug=True)
